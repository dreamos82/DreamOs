...
ssize_t read(int fildes, void * buf, size_t nbyte)
{
    ...
    {
        return mountpoint_list[mp_id].operations.read(fs_fd,
                                                      (char *) buf,
                                                      nbyte);
    }
    else
...
ssize_t write(int fildes, const void * buf, size_t nbyte)
{
    if (fd_list[fildes].flags_mask & O_RDWR)
    {
        ...
        int mp_id = fd_list[fildes].mountpoint_id;
        if (mountpoint_list[mp_id].operations.write != NULL)
        {
            int fs_fd = fd_list[fildes].fs_spec_id;
            mountpoint_list[mp_id].operations.write(fs_fd, (void *) buf, nbyte);
        }
        else
        {
