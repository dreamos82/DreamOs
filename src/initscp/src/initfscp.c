...
int main(int argc, char * argv[])
{
    ...
    else
    {
        ...
        else
        {
            ...
            i = 0;
            while (!strcmp(argv[j + 1], "-m") ||
                   !strcmp(argv[j + 1], "--mountpoint"))
            {
            ...
            for (i = 0; i + j < argc - 2; i++)
            {
                ...
                else
                {
                    if ((strpbrk(argv[i + j + 1], "/")) != NULL)
                        strcpy(headers[i + (j / 2)].fileName,
                               strtok(strrchr(argv[i + j + 1], '/'), "/"));
                    else
            ...
            for (i = 0; i + j < argc - 2; i++)
            {
                ...
                printf("\t\tFileName: %s Length: %d offset: %d\n",
                       headers[i + (j / 2)].fileName,
                       headers[i + (j / 2)].length,
                       headers[i + (j / 2)].offset);
            }
            printf("[ \033[1;33mDONE\e[0m ]\n\n");
            fclose(fsdest);
        }
